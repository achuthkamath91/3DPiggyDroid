...
void drawCurves()
{
	...
	{
		_dump_current_modelview();
		fprintf(mds->m_rayFile, "curve(Curve {\n" );
		_dump_current_material();
		fprintf(mds->m_rayFile, "}))\n");
	}
	else
	{
		...
		GLfloat ctrlpoints[4][4][3] = {
			{ { -1.0, -1.0, 4.0 }, { -0.5, -1.0, 2.0 },
			{ 0.5, -1.0, -1.0 }, { 1.0, -1.0, 2.0 } },
			{ { -1.0, -0.5, 1.0 }, { -0.5, -0.5, 3.0 },
			{ 0.5, -0.5, 0.0 }, { 1.0, -0.5, -1.0 } },
			{ { -1.0, 0.5, 4.0 }, { -0.5, 0.5, 0.0 },
			{ 0.5, 0.5, 3.0 }, { 1.0, 0.5, 4.0 } },
		...
		for (j = 0; j <= 1020; j++) {
			...
			for (i = 0; i <= 30; i++)
				glEvalCoord2f((GLfloat)i / 30.0, (GLfloat)j / 1020.0);
			glEnd();
			glBegin(GL_LINE_STRIP);
			for (i = 0; i <= 30; i++)
				glEvalCoord2f((GLfloat)j / 1020.0, (GLfloat)i / 30.0);
			glEnd();
		}
